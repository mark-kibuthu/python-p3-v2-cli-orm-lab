...
def create_employee():
    ...
    while True:
        ...
        if job_title:
            break
        print("Job title must be a non-empty string.")

    while True:
        department_id = input("Enter the department id: ").strip()
        if department_id.isdigit() and int(department_id) > 0:
            department_id = int(department_id)
            if Department.find_by_id(department_id):
                break
            else:
                print(f"Department with id {department_id} not found. Please try again.")
        else:
            print("Invalid department id. Please enter a valid positive integer.")
...
def update_employee():
    ...
    if employee := Employee.find_by_id(id_):
        try:
            ...
            if not department_id.isdigit() or not Department.find_by_id(int(department_id)):
